...
//  - - - -  Information Logging  - - - -  //
	...
	static void logGameInfo()
	{
		String toPrint = ""
				...
				+  norm(" | Lives : " 	+ ADVENT.lives)
				+  " | \n"
				+  norm(" | Inventory : " + ADVENT.itemsInHand + " " + GameObjects.objectsHere(Locations.INHAND))
				+  " | \n"
				+  norm(" | Objects Here: " + GameObjects.objectsHere(ADVENT.currentLocation)) + " | "
	...
	static String printQuestionsAndHintsStatus()
	{
		...
		{
			hints += hint != Hints.NONE ? norm(" | "+ hint.name +": "+ hint.cost +" "+ hint.proc +"/"+ hint.threshold +" "+ (hint.given ? "1 " : "0 ")) : "";
		}
	...
	{
		while(input.length() % LOG_SEGMENT_LENGTH != 0)
		{ input += " "; }
		return input;
	}
...
//  - - - -  Questions & Hints  - - - -  //
	...
	enum Hints
	{
		...
		INSTRUCTIONS("Intro", 	5,  -1, null, 													"Somewhere nearby is Colossal Cave, where others have found great fortunes in treasure and gold, though it is rumored that some who enter are never seen again. "
																								...
																								+ "I should warn you that I only look at only the first five letters of each word, so you'll have to enter \"northeast\" as \"ne\" to distinguish it from \"north\" "
...
/*6*/	WITT		("Witt"	,   3,  20, "\n\nDo you need help getting out of here?", 			"Don't go west."),
		...
		final String 	name, question, hint;
...
//  - - - -  Known Word Enums & Map  - - - -  //
	...
	public enum GameObjects implements KnownWord
	{
		...
		static boolean isTreasure(GameObjects thisThing)
		{ return thisThing.ordinal() > GameObjects.MOSS.ordinal(); }
		
		static boolean isLesserTreasure(GameObjects thisThing)
		...
		static ArrayList<GameObjects> objectsHere(Locations here)
		{
		...
		static String listItemsHere(Locations here)
		{
			String output = "";
	...
	enum ActionWords implements KnownWord
	{
	...
	enum MessageWords implements KnownWord
	{
		MAGIC("Good try, but that is an old worn-out magic word."), 
		HELP("I know of places, actions, and things. Most of my vocabulary describes places and is used to move you there. To move, try words like forest, building, downstream, enter, east, west, north, south, up, or down. "
					 + "I know about a few special objects, like a black rod hidden in the cave. These objects can be manipulated using some of the action words that I know. Usually you will need to give both the object and the action words (in either order), but sometimes I can infer the object from the verb alone. "
					 + "Some objects also imply verbs; in particular, \"inventory\" implies \"take inventory\", which causes me to give you a list of what you are carrying. The objects have side effects; for instance, the rod scares the bird. Usually people having trouble moving just need to try a few more words. "
					 + "Usually people trying unsuccessfully to manupulate an object are attempting something beyond their (or my!) capabilities and should try a completely different tack. To speed the game you can sometimes move long distances with a single word. "
					 + "For example, \"building\" usually gets you to the building from anywhere above ground except when lost in the forest. Also, note that cave passages turn a lot, and that leaving a room to the north does not guarantee entering the next from the south. \nGood luck!"),
		TREE("The trees of the forest are large hardwood oak and maple, with an occasional grove of pine or spruce. There is quite a bit of under-growth, largely birch and ash saplings plus nondescript bushes of various sorts. "
		...
		INFO("If you want to end your adventure early, say \"quit\". To get full credit for a treasure, you must have left it safely in the building, though you get partial credit just for locating it. You lose points for getting killed, or for quitting, though the former costs you more. "
					 ...
					 + "If you think you've found all the treasures, just keep exploring for a while. If nothing interesting happens, you haven't found them all yet. "
					 + "If something interesting DOES happen, it means you're getting a bonus and have an opportunity to garner many more points in the master's section. I may occasionally offer hints if you seem to be having trouble. "
					 + "If I do, I will warn you in advance how much it will affect your score to accept the hints. "
					 + "Finally, to save paper, you may specify \"brief\", which tells me never to repeat the full description of a place unless you explicitly ask me to. You may also specify \"verbose\", which tells me only to repeat the long description of a place."),
	...
	static final HashMap<String, KnownWord> KnownWords = new HashMap<String, KnownWord>()
	{{
...
//  - - - -  Location Enums  - - - -  //
	...
	public enum Locations
	{
		...
		EASTMIST(	"Hall Of Mists",		"You're in Hall of Mists.",				"You are at one end of a vast hall stretching forward out of sight to the west. There are openings to either side. Nearby, a wide stone staircase leads downward. The hall is filled with wisps of white mist swaying to and fro almost as if alive. "
		...
		ALIKE1(AdventMain.ALIKE_T, AdventMain.ALIKE_PASSAGES),	ALIKE2(AdventMain.ALIKE_T, AdventMain.ALIKE_PASSAGES),	ALIKE3(AdventMain.ALIKE_T, AdventMain.ALIKE_PASSAGES),	ALIKE4(AdventMain.ALIKE_T, AdventMain.ALIKE_PASSAGES),	ALIKE5(AdventMain.ALIKE_T, AdventMain.ALIKE_PASSAGES),
		ALIKE6(AdventMain.ALIKE_T, AdventMain.ALIKE_PASSAGES),	ALIKE7(AdventMain.ALIKE_T, AdventMain.ALIKE_PASSAGES),	ALIKE8(AdventMain.ALIKE_T, AdventMain.ALIKE_PASSAGES),	 ALIKE9(AdventMain.ALIKE_T, AdventMain.ALIKE_PASSAGES),	ALIKE10(AdventMain.ALIKE_T, AdventMain.ALIKE_PASSAGES),
		ALIKE11(AdventMain.ALIKE_T, AdventMain.ALIKE_PASSAGES),	ALIKE12(AdventMain.ALIKE_T, AdventMain.ALIKE_PASSAGES),	ALIKE13(AdventMain.ALIKE_T, AdventMain.ALIKE_PASSAGES),	ALIKE14(AdventMain.ALIKE_T, AdventMain.ALIKE_PASSAGES),
		...
		DIFF0	(AdventMain.DIFF_T, "You are in a maze of twisty little passages, all different."	),
		DIFF1	(AdventMain.DIFF_T, "You are in a maze of twisting little passages, all different."	),
		DIFF2	(AdventMain.DIFF_T, "You are in a little maze of twisty passages, all different."	),
		DIFF3	(AdventMain.DIFF_T, "You are in a twisting maze of little passages, all different."	),
		DIFF4	(AdventMain.DIFF_T, "You are in a twisting little maze of passages, all different."	),
		DIFF5	(AdventMain.DIFF_T, "You are in a twisty little maze of passages, all different."	),
		DIFF6	(AdventMain.DIFF_T, "You are in a twisty maze of little passages, all different."	),
		DIFF7	(AdventMain.DIFF_T, "You are in a little twisty maze of passages, all different."	),
		DIFF8	(AdventMain.DIFF_T, "You are in a maze of little twisting passages, all different."	),
		DIFF9	(AdventMain.DIFF_T, "You are in a maze of little twisty passages, all different."	),
		DIFF10	(AdventMain.DIFF_T, "You are in a little maze of twisting passages, all different."	),
		PONY	(AdventMain.DEAD_END_T, AdventMain.DEAD_END),
		...
		EASTWINDOW(	"Window",					"You're at window on pit.",				"You're at a low window overlooking a huge pit, which extends up out of sight.	A floor is indistinctly visible over 50 feet below. Traces of white mist cover the floor of the pit, becoming thicker to the right. "
		...
		EAST2PIT(	"East Twopit Room",			"You're at east end of Twopit Room.",	"You are at the east end of the twopit room. The floor here is littered with thin rock slabs, which make it easy to descend the pits. There is a path here bypassing the pits to connect passages from east and west. "
																						+ "There are holes all over, but the only big one is on the wall directly over the west pit where you can't get at it."	),
		...
		ABOVEP	(AdventMain.SECRET_CANYON, 		"You are in a secret N/S canyon above a sizable passage."	),
		...
		WESTWINDOW(	"Window",					"You're at window on pit.",				"You're at a low window overlooking a huge pit, which extends up out of sight. A floor is indistinctly visible over 50 feet below. Traces of white mist cover the floor of the pit, becoming thicker to the left. Marks in the dust around the window would seem to indicate that someone has been here recently. "
		...
		ABOVER	(AdventMain.SECRET_CANYON, 		"You are in a secret N/S canyon above a large room."	),
		...
		RESER	(true,	"Reservoir",			"You're at reservoir.",					"You are at the edge of a large underground reservoir. An opaque cloud of white mist fills the room and rises rapidly upward. The lake is fed by a stream, which tumbles out of a hole in the wall about 10 feet overhead and splashes noisily into the water somewhere within the mist. "
																						+ "The only passage goes back toward the south."	),
		
		SCAN1	(AdventMain.SECRET_CANYON, "You're in a secret canyon that exits to the north and east."	),
		SCAN2	(AdventMain.SECRET_CANYON, "You're in a secret canyon that exits to the north and east."	),
		SCAN3	(AdventMain.SECRET_CANYON, "You're in a secret canyon that exits to the north and east."	),
		SECRET	(AdventMain.SECRET_CANYON, "You're in secret E/W canyon above tight canyon.", "You are in a secret canyon, which here runs E/W. It crosses over an very tight canyon 15 feet below. If you go down you may not be able to get back up."	),
		...
		BOULDERS(AdventMain.DEAD_END_T, "The canyon runs into a mass of boulders --- dead end."	),
		...
		DEAD0(AdventMain.DEAD_END_T, AdventMain.DEAD_END),	DEAD1(AdventMain.DEAD_END_T, AdventMain.DEAD_END),	DEAD2(AdventMain.DEAD_END_T, AdventMain.DEAD_END),	DEAD3(AdventMain.DEAD_END_T, AdventMain.DEAD_END),
		DEAD4(AdventMain.DEAD_END_T, AdventMain.DEAD_END),	DEAD5(AdventMain.DEAD_END_T, AdventMain.DEAD_END),	DEAD6(AdventMain.DEAD_END_T, AdventMain.DEAD_END),	DEAD7(AdventMain.DEAD_END_T, AdventMain.DEAD_END),
		DEAD8(AdventMain.DEAD_END_T, AdventMain.DEAD_END),	DEAD9(AdventMain.DEAD_END_T, AdventMain.DEAD_END),	DEAD10(AdventMain.DEAD_END_T, AdventMain.DEAD_END),	DEAD11(AdventMain.DEAD_END_T, AdventMain.DEAD_END),
		...
		NEEND	(	"North East End",			"You're at NE end.",					"You are at the northeast end of an immense room, even larger than the Giant Room. It appears to be a repository for the \"Adventure\" program. "
																						+ "Massive torches far overhead bathe the room with smoky yellow light. Scattered about you can be seen a pile of bottles (all of them empty), a nursery of young beanstalks murmering quietly, a bed of oysters, a bundle of black rods with rusty stars on their ends, and a collection of brass lanterns. "
		...
		static String getDescription(Locations here, int brief)
		{
		...
		static void placeObject(GameObjects thing, Locations here)
		{
